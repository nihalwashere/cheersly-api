...
const service = async () => {
  try {
    logger.info("STARTING MONTHLY STATS CRON SERVICE");
    ...
    logger.error("MONTHLY STATS CRON SERVICE FAILED -> error : ", error);
  }
};
...
mongoose.connection.on("connected", async () => {
  try {
    logger.info("Connected to database from monthly stats cron service");
...
mongoose.connection.on("error", (error) => {
  logger.error(
    "Database error from monthly stats cron service -> error : ",
    error
  );
});
